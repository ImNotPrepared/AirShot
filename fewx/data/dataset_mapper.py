...
class DatasetMapperWithSupport:
    ...
    def __init__(self, cfg, is_train=True):
        if cfg.INPUT.CROP.ENABLED and is_train:
            ...
            logging.getLogger(__name__).info("CropGen used in training: " + stxrr(self.crop_gen))
        else:
            self.crop_gen = None
        ...
        if self.is_train:
            ...
            if self.few_shot:
                self.support_df = pd.read_pickle("./datasets/coco/{}_shot_support_df.pkl".format(self.support_shot+1)) # val
            else:
                self.support_df = pd.read_pickle("./datasets/coco/voc_val_support_df.pkl") # voc_val, train

            metadata = MetadataCatalog.get('coco_2017_val_voc')#('coco_2017_train') 'coco_2017_val_voc'
    ...
    def generate_support(self, dataset_dict):
        ...
        # coco: {'iscrowd': 0, 'bbox': [268.34, 0.86, 211.66, 213.81], 'category_id': 25, 'id': 280450, 'bbox_mode': <BoxMode.XYWH_ABS: 1>}
        id = dataset_dict['annotations'][0]['id']
        #print(self.support_df.loc[self.support_df['id']==id, 'category_id'].tolist())
        ...
        for shot in range(support_shot):
            ...
            # If >=2 shots, support from different (shot-1) images
            support_id = self.support_df.loc[(self.support_df['category_id'] == query_cls) & (~self.support_df['image_id'].isin(used_image_id)) & (~self.support_df['id'].isin(used_id_ls)), 'id']
            #if (support_id.shape[0]==0):
            #    print('pass sittuation:', query_cls,) 
            #    continue
            support_id = support_id.sample(random_state=id).tolist()[0]
            # If =1 shot, support from the same image
            # support_id = id
            ...
            support_data = utils.read_image(support_db["file_path"].tolist()[0], format=self.img_format) # val
        ...
        if support_way == 1:
            ...
            for way in range(support_way-1):
                ...
                for shot in range(support_shot):
                    ...
                    support_id = self.support_df.loc[(self.support_df['category_id'] == other_cls) & (~self.support_df['image_id'].isin(used_image_id)) & (~self.support_df['id'].isin(used_id_ls)), 'id']
                    if (support_id.shape[0]==0):
                        continue
                    support_id = support_id.sample(random_state=id).tolist()[0]
