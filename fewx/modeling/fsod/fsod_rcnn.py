...
class FsodRCNN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        for i in range(B): # batch
            ...
            ### revise  interaction with query_feature_res * pos_correlation_4 (ATTENTION MAP * )
            # highly_active_region = softmax(pos_correlation_2)[MASK]   # 
            # P_qf = masked_averaged_pooling( , )
            # pos_support_features_pool_4.permute(1,0,2,3)
            # revised_prototype = 0.5 * pos_support_features_pool_4.permute(1,0,2,3) + 0.5 * P_qf
            # revised_prototype = ...
            
            
            # repeat for 3,4 
            #pos_correlation_3 = pos_correlation_3.mean(dim=[2, 3], keepdim=True)
            #pos_correlation_4 = pos_correlation_4.mean(dim=[2, 3], keepdim=True)
            # repeat for 3,4
            # revise
            ...
            # revision
            p_proposals = self.roi_heads.label_and_sample_proposals( pos_proposals , query_gt_instances)
            pos_proposals_boxes = [proposal.proposal_boxes for proposal in p_proposals]
            pos_box_features = self.roi_heads._shared_roi_transform(
                [pos_features['res4']], pos_proposals_boxes
            )
            ...
            # revision_1
            print(pos_gt_boxes[0].shape)
            print(neg_gt_boxes[0].shape)
            n_proposals = self.roi_heads.label_and_sample_proposals(neg_proposals , query_gt_instances)
            neg_proposals_boxes = [proposal for proposal in n_proposals]
            neg_box_features = self.roi_heads._shared_roi_transform(
                [neg_features['res4']], neg_proposals_boxes
            )      
            box_features = torch.cat((pos_box_features, neg_box_features), dim=0)
            logits, labels  = self.info_nce_loss(box_features)
            ...
            if self.training:
                # revision  
                proposal_losses, contrastive_loss = self.proposal_generator.losses(
            ...
            rpn_loss_rpn_cls_pos.append(proposal_losses['pos_loss'])
            rpn_loss_rpn_cls_neg.append(proposal_losses['neg_loss'])
            ...
            detector_loss_cls_pos.append(detector_losses['POS_Loss'])
            detector_loss_cls_neg.append(detector_losses['NEG_Loss'])
            
            detector_loss_box_reg.append(detector_losses['loss_box_reg'])
            
            #self.visualize_training(batched_inputs[i], pos_proposals[0], mode = 'Pos_proposals', )
            #self.visualize_training(batched_inputs[i], neg_proposals[0], mode = 'Neg_proposals', )
            #self.visualize_training(batched_inputs[i], pos_detector_proposals[0], mode = 'Pos_detector')
        ...
        contrastive_losses['simCLR'] = contrastive_loss
        proposal_losses['pos_loss_rpn'] = torch.stack(rpn_loss_rpn_cls_pos).mean()
        proposal_losses['neg_loss_rpn'] = torch.stack(rpn_loss_rpn_cls_neg).mean()
        ...
        detector_losses['POS_Loss'] = torch.stack(detector_loss_cls_pos).mean() 
        detector_losses['NEG_Loss'] = torch.stack(detector_loss_cls_neg).mean() 
        #detector_losses['BG_Loss'] = torch.stack(detector_loss_cls_bg).mean()       
    ...
    def info_nce_loss(features):
        ...
        # (256, 2048, 7, 7)
        box_features = F.normalize(features, dim=1)
        similarity_matrix = torch.matmul(box_features, box_features.T)
