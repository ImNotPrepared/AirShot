...
class FsodRPN(nn.Module):
    ...
    def losses(
        ...
        """
        #rev ision
        #gt_labels, gt_pos_labels, gt_neg_labels =  gt_labels[:2], gt_labels[0], gt_labels[2]
        ...
        #rev ision
        #gt_objectness_labels = torch.stack([gt_pos_labels] + [gt_neg_labels])
        #real_pos_mask = gt_pos_labels == 1
        #real_neg_mask = gt_neg_labels == 1
        #before_pos_objectness, before_neg_objectness = cat(pred_objectness_logits, dim=1)[0, ...], cat(pred_objectness_logits, dim=1)[1, ...]
        ...
        valid_mask = gt_labels >= 0  # [2,260011]
        neg_mask = gt_labels == 0

        # calculate pos/neg loss  Label values are in {-1, 0, 1}, with meanings: -1 = ignore; 0 = negative class; 1 = positive class.
        pos_gt_mask = gt_labels[pos_mask].to(torch.float32)
        neg_gt_mask = gt_labels[neg_mask].to(torch.float32)
        pos_objectness = cat(pred_objectness_logits, dim=1)[pos_mask]
        neg_objectness = cat(pred_objectness_logits, dim=1)[neg_mask]
        
        #### rev ision
        #temp = 0.9
        #pred_pos_objectness = before_pos_objectness[real_pos_mask] # [ num of proposals ]
        #pred_neg_objectness = before_neg_objectness[real_neg_mask]
        
        pos_loss = F.binary_cross_entropy_with_logits(pos_objectness, pos_gt_mask, reduction="sum",)
        neg_loss = F.binary_cross_entropy_with_logits(neg_objectness, neg_gt_mask, reduction="sum",)
        
        # pred_objectness_logits： representing the predicted objectness logits for all anchors.
        ...
        #    contrastive_loss = self.contrastive_loss[0]
        #    self.contrastive_loss=[]
        #else:
        #    contrastive_loss= self.feature_store[0]

        return {
            "pos_loss":  pos_loss/num_pos_anchors,
            "neg_loss":  neg_loss/num_neg_anchors,    
            "loss_rpn_cls": objectness_loss / normalizer,
            "loss_rpn_loc": localization_loss / normalizer,
        }, contrastive_loss  #revision
    ...
    def forward(
        ...
        """
        features = [features[f] for f in self.in_features] # (1, 1024, h, w)
